...
function trans_list_pages($args = ''){
	...
	foreach ($pages as $page){
		...
		$output.='<a href="'.get_permalink($page->ID).'" title="'.wp_specialchars($page->post_title).'">'.'<li id="transMenuTop_'.$page->ID.'" '.$cur_element.'>'.$page->post_title.'</li></a>';
	}
	//$output.='';
	echo $output;
}
...
function trans_get_pages($args = '') {
	...
	$pages = $wpdb->get_results("SELECT * " .
		"FROM $wpdb->posts " .
		"WHERE post_type = 'page' AND post_parent = ". $r['child_of'] ." ".
		"$exclusions " .
